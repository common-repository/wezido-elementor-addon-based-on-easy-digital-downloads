...
class wezido {
    ...
    private function wezido_load_dependencies() {
        ...
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-wezido-admin.php';
         ...
         /**
         * The files reponsible for admin options
         */

        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/vendor/codestar-framework/codestar-framework.php';
        
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/wezido-extend-fields.php';
        
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/wezido-admin-options.php';
        
         require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/wezido-taxonomy.php';
    ...
    private function wezido_load_elementor_widgets_dependencies() {
        ...
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-counter.php';

        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-post.php';

        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-blog-category.php';

        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-edd-recent.php';
        
         require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-edd-masonry.php';
         
         require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-edd-justified.php';
         
         require_once plugin_dir_path( dirname( __FILE__ ) ) . 'elementor-widgets/class-edd-category.php';
    }
    ...
    public function wezido_register_elementor_widgets() {
        ...
        $wztitle= wezido_get_option( 'wezido_title',true );
        $wzteam= wezido_get_option( 'wezido_team',true );
        $wzflipbox= wezido_get_option( 'wezido_flipbox',true );
        $wzbeforeafter= wezido_get_option( 'wezido_before_after',true );
        $wzpricing= wezido_get_option( 'wezido_pricing_table',true );
        $wzcounter= wezido_get_option( 'wezido_counter',true );
        $wzpost= wezido_get_option( 'wezido_blog',true );
        $wzpostcat= wezido_get_option( 'wezido_blog_cat',true );
        $wzeddrecent= wezido_get_option( 'wezido_edd_recent',true );
        $wzeddmasonry= wezido_get_option( 'wezido_edd_masonry',true );
        $wzeddjustified= wezido_get_option( 'wezido_edd_justified',true );
        $wzeddcat= wezido_get_option( 'wezido_edd_cat',true );
        ...
        if ($wzpricing){
         ...
         if ($wzcounter){
            Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Wezido_counter() );
        }
        ...
        if ($wzpost){
         ...
         if ($wzpostcat){
            Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Wezido_blog_catgeory() );
        }
        ...
        if (class_exists('Easy_Digital_Downloads')){
            ...
            if ($wzeddmasonry){
                Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Wezido_edd_masonry() );
            }
            
             if ($wzeddjustified){
                Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Wezido_edd_justified() );
            }
            
             if ($wzeddcat){
                Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Wezido_edd_category() );
            }
