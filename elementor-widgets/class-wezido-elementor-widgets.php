...
class wezido_Elementor_Widgets {
	...
	public function wezido_enqueue_scripts() {
		 ...
		 * class.
		 */
         $wzbeerjs= wezido_get_option( 'wezido_beer_slider',true );
         $wzbeafjs= wezido_get_option( 'wezido_before_after_js',true );
         $wzflipjs= wezido_get_option( 'wezido_flipbox_js',true );
         $wzcupjs= wezido_get_option( 'wezido_counterup',true );
         $wzcujs= wezido_get_option( 'wezido_counter_js',true );
         $wzgzjs= wezido_get_option( 'wezido_gridzy_js',true );
         
	    
	    if ($wzbeerjs){
	     wp_enqueue_script( 'wezido_beerslider' );
	    }
	    if($wzflipjs){
	     ...
	     if($wzcupjs){
	     wp_enqueue_script( 'wezido_counterup' );
	     }
	     
	     if($wzcujs){
	     wp_enqueue_script( 'wezido_counter' );
	     }
	     
	     if($wzgzjs){
	     wp_enqueue_script( 'wezido_gidzy' );
	     }
	}
	   ...
	   /**
        ...
        public function wezido_get_scripts(){
            ...
            $script_list = [
                'wezido_beerslider' => [
                    'src'     => plugin_dir_url( __FILE__ ) . 'js/vendor/jquery.beerslider.js',
                    'version' => 1.1,
                    'deps'    => [ 'jquery' ]
                ],
                
                'wezido_counterup' => [
                    'src'     => plugin_dir_url( __FILE__ ) . 'js/vendor/jquery.counterup.min.js',
                    'version' => 1.1,
                    'deps'    => [ 'jquery' ]
                ],
                'wezido_gidzy' => [
                    'src'     => plugin_dir_url( __FILE__ ) . 'js/vendor/jquery.min.gidzy.js',
                    'version' => 1.1,
                    'deps'    => [ 'jquery' ]
                ],
                ...
                'wezido_before_after' => [
                 ...
                 'wezido_counter' => [
                    'src'     => plugin_dir_url( __FILE__ ) . 'js/wezido-counter.js',
                    'version' => 1.1,
                    'deps'    => [ 'jquery' ]
                ],
