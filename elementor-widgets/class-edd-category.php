 ...
 * @since 1.0.0
 */
class Wezido_edd_category extends Elementor\Widget_Base {
    ...
    protected function _register_controls() {
        $this->start_controls_section(
            'section_content',
            [
                'label' => __( 'Wezido EDD Catgeory', 'wezido' ),
            ]
        );
    ...
    protected function render() {
        ...
        $categories=get_categories(
            array('taxonomy' =>'download_category' ,'number' => $cats_count, 'order'   => (string)trim($cat_order_term),)
        );
        ?>

        <?php if($cat_style=="grid"){?>
            <div class="wezido-edd-category-grid flex flex-wrap overflow-hidden sm:-mx-<?php echo esc_html($grid_gap);?> lg:-mx-<?php echo esc_html($grid_gap);?> xl:-mx-<?php echo esc_html($grid_gap);?>">
                <?php foreach ( $categories as $term ) : ?>
                    <?php
                    $wizcat_tax = get_term_meta($term->term_id, 'wezido_edd_taxonomy', true );
                     $wiz_cat_bg = $wizcat_tax['wiz_edd_cat_image']['url'];
                    ?>
